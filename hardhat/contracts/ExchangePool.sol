...
contract ExchangePool{
    ...
    */
    function provideLiquidity(uint _amountNoi, uint _amountDai/*, uint _percentError*/) external returns (uint amountA, uint amountB, uint liquidity){
        return IRouter02(poolRouterAddr).addLiquidity(
            noiAddr,
            daiAddr,
            _amountNoi,
            _amountDai,
    ...
    function removeLiquidity(uint _liquidity /*, uint minNoi, uint minDai */) external returns (uint amountNoi, uint amountDai){
        return IRouter02(poolRouterAddr).removeLiquidity(
            noiAddr,
            daiAddr,
            _liquidity,
            0,                          //minNoi,
    ...
    function exchangeNoiForDai(uint _amount) external returns (uint inputAmount, uint outputAmount){
        address[] memory tokenAddresses;
        tokenAddresses[0] = noiAddr;
        tokenAddresses[1] = daiAddr;

        uint[] memory returnAmounts = IRouter02(poolRouterAddr).swapExactTokensForTokens(
            _amount,
            0,
            tokenAddresses,
            address(msg.sender),
    ...
    function exchangeDaiForNoi(uint _amount) external returns (uint inputAmount, uint outputAmount){
        address[] memory tokenAddresses;
